...
1. 打开后台登录页 → 填账号密码 → 登录
2. 落点判断：无品牌 → 官网 `/trial`（吸收 `#token`）→ 提交品牌 1 → 等 SSE 分析 → 确认监控问题 → 报告卡 → 回控制台；有品牌 → 直接进后台
3. **可选多品牌**：任务填了「品牌 2」时，在控制台点品牌切换 →「添加新品牌」→ `/trial?from=add_brand` 再建第二品牌 → 回控制台 → 断言切换面板有 2 个品牌
4. 后台逐页验证采集前真实数据：概览 / 套餐（免费体验版·4 档）/ 名片（品牌名·剩余修改）/ 口碑监控问题 / 识别管理 / 排名监控问题 / 信源库
5. 每步写日志 + 截图；执行结束记录 `user_id`（JWT sub）与 `brand_id` 作为删除锚点
