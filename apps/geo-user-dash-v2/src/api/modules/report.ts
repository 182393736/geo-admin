...
import type { ReportLatestResp, ReportCycleResp, MediaListResp, PublishOrder } from '../types'

export const reportApi = {
  cycle: () => get<ReportCycleResp>('/report/cycle'),
  list: (period_type: 'weekly' | 'monthly') =>
    post('/report/list', { period_type, limit: 8, offset: 0 }),
  latest: (period_type: 'weekly' | 'monthly' = 'weekly') =>
    post<ReportLatestResp>('/report/latest', { period_type }),
}

export const publishApi = {
  mediaFacets: () =>
    post<{
      taxonomy: { value: string; count: number }[]
      areas?: { value: string; count: number }[]
      types?: { value: string; count: number }[]
    }>('/publish/media/facets', {}),
  ...
  drafts: (payload: Record<string, any> = {}) =>
    post<{ list: any[]; total: number; page: number }>('/publish/article/drafts', { size: 50, ...payload }),
  ...
  estimate: (mediaKeys: string[], articleId?: string) =>
    post<{ total_points: number; list: any[]; count: number }>('/publish/estimate', {
