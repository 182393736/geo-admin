  ...
  >
    ...
    >
      <header class="flex items-start gap-3 border-b border-gray-100 px-5 py-4">
        <div class="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary">
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="flex flex-wrap items-center gap-2 border-b border-gray-100 bg-gray-50/70 px-5 py-3">
          <label class="flex h-9 min-w-[190px] flex-1 items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 focus-within:border-ring">
          ...
          <template v-if="selectedCount > 0">
            ...
            <select v-model="batchAssign" class="h-9 max-w-[190px] rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-700 outline-none focus:border-ring">
        ...
        <div class="min-h-[240px] flex-1 overflow-y-auto">
          ...
          >
            ...
            <select
              ...
              :class="row.assignment_type === 'pending' ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-gray-200 bg-white text-gray-700 focus:border-ring'"
      ...
      <footer class="border-t border-gray-100 bg-white px-5 py-4">
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <button type="button" class="flex h-9 items-center gap-1.5 rounded-lg bg-primary px-4 text-xs font-bold text-primary-foreground hover:bg-primary/90 disabled:opacity-50" :disabled="saving" @click="save">
              {{ saving ? '保存中...' : '保存归类' }}
            </button>
          </div>
        </div>
