...
class AgentController extends Controller {
    ...
    try {
      if (b.save === false) {
        ...
        const out = await ctx.service.agentRunner.runAndPersist(userId, input, {
          brandId: b.brand_id,
          selectedQueries: b.selected_queries,
          selectedAliases: Array.isArray(b.selected_aliases) ? b.selected_aliases.map(String) : undefined,
          confirmLimit: b.confirm_limit,
        });
   ...
   * 请求体：{ preview, brand_name?, selected_queries: string[], selected_aliases?: string[], brand_id?, confirm_limit? }
   * confirm_limit 服务端硬封顶 = config.geoAgent.freeQueryLimit（默认 3）
   * brand_name 用户确认后的正式品牌词；不传则用 preview.brand.name
   * selected_aliases 为用户确认后的识别名（可增删改）；不传则沿用 preview.aliases
   */
  async confirm() {
    const { ctx } = this;
    const b = ctx.request.body || {};
    ...
    const brandName = typeof b.brand_name === 'string' ? String(b.brand_name).trim().slice(0, 60) : undefined;
    try {
