...
export interface PlanItem {
  ...
  original_price: number; price: number; credit_price: number;
  discount_rate?: number; query_limit: number;
  platform_list?: string[]; platform_scope?: string;
  features: unknown; sort: number; on_sale: boolean;
}

export interface PlansGrouped {
  free: PlanItem[]; starter: PlanItem[]; basic: PlanItem[];
  pro: PlanItem[]; custom: PlanItem[];
}

export interface SubscriptionFull extends Subscription { status?: string }

export interface CreditTxn {
  txn_id: string; type: string; coin: string; amount: number;
  balance_after?: number; ref_type?: string; ref_id?: string;
  remark?: string; title?: string; created_at: string;
}

export interface RechargePack {
  pack_id: number; credits: number; price: number; unit_price: number;
}

export interface PayOrderCreated {
  order_no: string; status: string; pay_method: string;
  plan_code?: string; plan_name?: string; plan_type?: string;
  price: number; credit_price?: number; query_limit?: number;
  duration_days?: number; expire_time?: string; code_url?: string;
  mock_pay?: boolean; credits?: number; pack_id?: number;
  paid_at?: string;
}
...
export const paymentApi = {
  ...
  createOrder: (body: { brand_id?: string; plan_code: string; pay_method: 'wx' | 'credit'; order_type?: string }) =>
    post<PayOrderCreated>('/payment/order/create', body),
  getOrder: (orderNo: string) => get<PayOrderCreated>(`/payment/order/${orderNo}`),
  mockPay: (orderNo: string) => post<{ order_no: string; status: string }>(`/payment/order/${orderNo}/mock-pay`),
};

export const creditApi = {
  account: () => get<{
    balance: number; available: number; frozen: number;
