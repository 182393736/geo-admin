'use strict';
/**
 * geo-test 服务：测试任务列表 + 添加任务 + 执行（Playwright）+ 删除任务数据。
 * 端口 PORT（默认 6005），网页在 /（public/index.html），API 前缀 /api。
 * 启动前加载 apps/geo-test/.env（由 pnpm env:local 写出）。
 */
...
app.post('/api/tasks', wrap(async (req, res) => {
  const { account, password, brandInput, brandInput2, screenshot, headed, slotDays } = req.body || {};
  if (!account || !password || !brandInput) {
    return res.status(400).json({ code: 400, msg: '账号、密码、品牌输入均必填' });
  }
  // 账号预检：不存在则插入；已存在则同步密码（与 geo-api 同库 geo）
  try {
  ...
  const t = await createTask({ account, password, brandInput, brandInput2, screenshot, headed, slotDays });
...
app.use((err, req, res, next) => {
  console.error('[geo-test]', err);
  res.status(500).json({ code: 500, msg: String((err && err.message) || err).slice(0, 300) });
});

connect().then(() => {
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[geo-test] 测试程序已启动: http://0.0.0.0:${PORT}（Mongo: ${require('./db').MONGO_URL}）`);
    console.log(`[geo-test] DASH=${process.env.DASH_URL || 'http://127.0.0.1:5180'} SITE=${process.env.SITE_URL || 'http://localhost:5003'} API=${process.env.API_URL || 'http://127.0.0.1:6001'}`);
  });
}).catch(e => {
  console.error('[geo-test] MongoDB 连接失败:', e.message);
  process.exit(1);
});
