...
const DASH = process.env.DASH_URL || 'http://127.0.0.1:5180';
const SITE = process.env.SITE_URL || 'http://localhost:5003';
...
/** 依赖服务健康检查（DASH / API / ADMIN 必选；SITE 可选探活） */
async function checkDeps() {
  const required = { DASH, API, ADMIN };
  const optional = { SITE };
  const missing = [];
  const offlineOptional = [];
  for (const [k, base] of Object.entries(required)) {
    try {
  ...
  for (const [k, base] of Object.entries(optional)) {
    try {
      const r = await fetch(base, { signal: AbortSignal.timeout(5000) });
      if (!r.ok && r.status >= 500) offlineOptional.push(`${k}(${base})`);
    } catch {
      offlineOptional.push(`${k}(${base})`);
    }
  }
  return {
    ok: missing.length === 0,
...
async function runTask(taskId) {
  ...
  try {
    ...
    // 与 geo-api 同库同步账号密码，避免 geo_dev / 过期密码导致登录失败
    ...
    if (!depCheck.ok) {
      await updateTask(taskId, {
        ...
        result: { pass: false, summary: `依赖服务未就绪：${depCheck.missing.join('、')}（请先启动 geo-api(:6001) / dash-v2(:5180) / site-manage web(:5003) / geo-admin(:6004)）` },
      });
      return;
    }
