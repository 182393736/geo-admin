...
function createClient() {
  const cfg = getOssConfig();
  if (!cfg.enabled) {
    throw new Error('OSS 未配置：请设置环境变量或创建 apps/geo-caiji/oss.local.json（参考 oss.local.example.json）');
  }
  if (!OSS) throw new Error('未安装 ali-oss，请在 apps/geo-caiji 执行 pnpm add ali-oss');
  return new OSS({
