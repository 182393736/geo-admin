...
pnpm --filter @geo-admin/geo-caiji install:browsers

# 3. 启动（vite 渲染层 + electron 主进程一起拉起）
pnpm --filter @geo-admin/geo-caiji dev
# 或根目录：
pnpm dev:caiji
```
...
- 每个 IP 的浏览器用户数据目录：`app.getPath('userData')/profiles/<ip>`
  - Windows：`%APPDATA%/geo-caiji/profiles/<ip>`
  - macOS：`~/Library/Application Support/geo-caiji/profiles/<ip>`
  - Linux：`~/.config/geo-caiji/profiles/<ip>`

## 后续扩展点（代码中已留注释）

- 代理注入：`main.cjs` 的 `launchPersistentContext` 处注入 `proxy: { server: \`http://${ip}:${port}\` }`
- 采集拉取/提交：对接 geo-api 的 `/collector/slots/pull`、`/collector/slots/:slot_id/submit`
